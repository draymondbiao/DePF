...
def get_train_images_auto(paths, height, width, mode='RGB'):
    if isinstance(paths, str):#判断一个对象的变量类型。
        paths = [paths]
    images = []
    for path in paths:
        ...
        image = tensor_trans(image)  # 将PIL转换成tensor
        vis_y_image, vis_cb_image, vis_cr_image = RGB2YCrCb(image)
        images.append(vis_y_image)
    ...
    images = np.stack(images, axis=0)#行叠堆
    images = torch.from_numpy(images).float()#把数组转换成张量tensor,且二者共享内存
    return images
