...
def L1_Norm(source_imgs1,source_imgs2)->torch.FloatTensor:
    ...
    mask_value = l1_b_ + l1_a_ + 1e-10
    mask_sign_a = torch.divide(l1_a_, mask_value)#除法
    mask_sign_b = torch.divide(l1_b_, mask_value)
    for i in range(dimension[1]):
        temp_matrix = torch.mul(mask_sign_a, source_imgs1[0, i, :, :]) + torch.mul(mask_sign_b,
                                                                                   source_imgs2[0, i, :, :])
...
def LEGM(source_imgs1,source_imgs2)->torch.FloatTensor():
    ...
    E_1 = F.conv2d(s1, weight, stride=1, padding=1, groups=dimension[1])
    E_2 = F.conv2d(s2, weight, stride=1, padding=1, groups=dimension[1])
    h_sobel = torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=source_imgs1.dtype, device=device).view(1, 1, 3,
                                                                                                               3)
    v_sobel = torch.tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype=source_imgs1.dtype, device=device).view(1, 1, 3,
                                                                                                               3)
    h_sobel = torch.cat([h_sobel] * dimension[1], dim=0)
    v_sobel = torch.cat([v_sobel] * dimension[1], dim=0)
    grad_img1 = torch.conv2d(source_imgs1, h_sobel, stride=1, padding=1, groups=dimension[1]) ** 2 + torch.conv2d(
        source_imgs1, v_sobel, stride=1, padding=1, groups=dimension[1]) ** 2
    ...
    five_adjacent = torch.tensor([[0, 1, 0], [1, 1, 1], [0, 1, 0]], dtype=source_imgs1.dtype, device=device).view(1, 1,
                                                                                                                  3, 3)
...
# channel attention
def channel_attention(tensor, pooling_type='attention_nuclear'):
    # global pooling
    shape = tensor.size()
...
def new_fusion_rule(src1,src2,device)->torch.FloatTensor:
    vgg_model = VGG19(device=device)
    # src1 = cv2.imread(source_img1_name)
    # src2 = cv2.imread(source_img2_name)
    if not isGray(src1):
        # src1 = cv2.cvtColor(src1, cv2.COLOR_BGR2YCrCb)
        src1_new = src1[:, :, 0]
    else:
        src1_new = src1
    if not isGray(src2):
        # src2 = cv2.cvtColor(src2, cv2.COLOR_BGR2YCrCb)
        src2_new = src2[:, :, 0]
    else:
        src2_new = src2
    # 转为torch tensor
...
def PA_PCNN(matrix,args)->np.ndarray:
    ...
    for i in range(iteraion_times):
        K = cv2.filter2D(np.uint8(Y), ddepth=cv2.CV_32F, kernel=W, borderType=cv2.BORDER_REFLECT)
...
#     IR=cv2.imread(r'../test_data/MSRS/Inf/00537D.png')
#     VIS=cv2.imread(r'../test_data/MSRS/Vis/00537D.png')
#     IR=cv2.resize(IR,dsize=(256,256),interpolation=cv2.INTER_CUBIC)
#     VIS=cv2.resize(VIS,dsize=(256,256),interpolation=cv2.INTER_CUBIC)
#     IR_ycrcb=cv2.cvtColor(IR,cv2.COLOR_BGR2YCrCb)
...
#     model = Net()
#     model_path = r'../model_checkpoints/ckpt_10_0.5.pth'
#     model1_path = r'../model_checkpoints/ckpt_10_0.5.pth'
#     with torch.no_grad():
...
#         cv2.imwrite('./results/focus.jpg',result)
